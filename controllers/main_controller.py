...
from controllers.PlaySoundEnvA import sine_tone_EnvA
import tables

inputmat = tables.openFile('C:/Users/Dalmau/Documents/Python Scripts/OutputMatrixPorts9Batch.mat')
...
RewNonRewTimeRatio = float(
    TimesRecallVector[ExpDay - 1][0]) / 10.  # 10./10. #1./2. #Time Ratio of the experiment until reach rewarded trials
MinPerf = 0.85
MinNumTrials = 20
FlagRandDeliver = 1
...
p1 = Process(target=video_controller.runvideo, args=(running, isRecording, Xmean, Ymean, XTA, YTA, RTA, Xport, Yport, Xcirc, Ycirc,
...
while time.time() <= (t_end + 0.001):  # and (np.mean(AccumPerf[-MinNumTrials:])<MinPerf):
    ...
    if ((t_init + aclimTimeLength * 60.) < time.time()) and (np.sqrt(np.square(Xmean.value - centerDiskX) + np.square(
    ...
    Aux2 = np.array(dataPlotIncorrectPort,
                    dtype='float')  # Accumulative sum of licks before getting to the correct port
