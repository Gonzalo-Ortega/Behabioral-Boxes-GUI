...
def runvideo(running, isRecording, Xmean, Ymean, XTA, YTA, RTA, Xport, Yport, Xcirc, Ycirc, Rcirc, ):
    ...
    while running.value == 1:
        ...
        if h is None:
            (h, w) = frame.shape[:2]
        frameSmall = imutils.resize(frame,
                                    width=320)  # defaulte frame is H=640,W0720, resize to fit on the screen W=300
        ...
        if 0 < len(Xmask) and 0 < len(Ymask):
            if (np.sqrt(np.square(int(np.floor(np.mean(Xmask))) - Xcirc.value) + np.square(
                ...
                Ym[0] = int(np.floor(np.mean(
                    Ymask)));  # Xm[9]=Xm[8];Ym[9]=Ym[8];Xm[8]=Xm[7];Ym[8]=Ym[7];Xm[7]=Xm[6];Ym[7]=Ym[6];Xm[6]=Xm[5];Ym[6]=Ym[5];Xm[5]=Xm[4];Ym[5]=Ym[4];Xm[4]=Xm[3];Ym[4]=Ym[3];Xm[3]=Xm[2];Ym[3]=Ym[2];Xm[2]=Xm[1];Ym[2]=Ym[1];Xm[1]=Xm[0];Ym[1]=Ym[0];Xm[0]=int(np.floor(np.mean(Xmask)));Ym[0]=int(np.floor(np.mean(Ymask)));
        # plot acircle around the center point of the contour
        cv2.circle(frame, (Xmean.value, Ymean.value), 25, (255, 0, 255), 1, 8,
                   0)  # cv.Circle(img, center, radius, color, thickness=1, lineType=8, shift=0)
        cv2.circle(frame, (XTA.value, YTA.value), int(round(RTA.value)), (0, 0, 255), 1, 14,
                   0)  # cv.Circle(img, center, radius, color, thickness=1, lineType=8, shift=0)
