...
import cv2
from mouseTrackerBox2MOG2AdapRecall import runvideo
...
#Global variables
toneFreq =7040 #10560 #7040 #4400 #In herzs IMPORTANT: SPEAKER AMPLITUDE AT 10
...
compensationFactor = (2.5, 1.43, 1.25, 1.43, 1.5, 1.43, 2.5, 1.11) #(0.73, 0.73, 0.67, 0.77, 0.68, 0.86, 0.71, 0.76) #(0.74, 0.79, 0.68, 0.77, 0.88, 0.96, 0.70, 0.79)#(0.7, 0.77, 0.7, 0.7, 0.75, 0.63, 0.64, 1)
...
FlagForRecallTest=1 #0FLAG TO TEST RECALL
FixMinRewTime = 7.#1.5
StageLevel =3
...
while time.time() <= (t_end +0.001):
    ...
    if (1<len(datatimes['timeLickCorrectPort'])) and (datatimes['cueTime'][-1]<datatimes['timeLickCorrectPort'][-1]):
