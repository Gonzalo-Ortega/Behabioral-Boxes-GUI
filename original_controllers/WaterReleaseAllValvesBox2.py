...
compensationFactor = (2.5, 1.43, 1.25, 1.43, 4., 1.43, 2.5, 1.11) #(0.72, 0.73, 0.74, 0.88, 0.70, 0.70, 0.64, 0.81) #(1, 1, 1, 1, 1, 1, 1, 1) #(0.71, 0.74, 0.69, 0.74, 1.44, 0.68, 0.66, 0.82) #(1, 1, 1, 1, 1, 1, 1, 1) #(1.98, 1.96, 1.88, 1.5, 1.32, 1.86, 1.37, 1.36)#(1, 1, 1, 1, 1, 1, 1, 1)#(1.3, 1.7, 1.3, 1.2, 1.4, 1.3, 1, 2.5) #(1.3, 1.7, 1.3, 1.2, 1.4, 1.3, 1, 2.5)  
##COMPENSANTION FACTOR WAS CALIBRATED THE 20/12/2021
...
for j in range(0,3):#if you want to loop 100 times write 100 here instead of 1
    for  i in range(0,(len(listofPorts))):# [(6-1)]:# range(0,(len(listofPorts))):#in [(5-1)]:#in range(0,(len(listofPorts))):# in [(2-1)]:# in range(0,(len(listofPorts))):# in [(5-1)]:# in [(8-1)]:# ## #in [(3-1)]:
        time.sleep(0.5)
