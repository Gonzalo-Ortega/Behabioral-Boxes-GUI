...
import cv2
from mouseTrackerBox2MOG2Adap import runvideo
from PlaySoundEnvB import sine_tone_EnvB
import tables
#inputmat=scipy.io.loadmat('C:/Users/Dalmau/Documents/Python Scripts/OutputMatrixPorts12Batch.mat')
#PortsMatrix=inputmat['A'].T
...
#Global variables
toneFreq =7040 #4400 #In herzs IMPORTANT: SPEAKER AMPLITUDE AT 30
...
compensationFactor = (2.5, 1.43, 1.25, 1.43, 1.5, 1.43, 2.5, 1.11) # (0.74, 0.79, 0.68, 0.77, 0.88, 0.96, 0.70, 0.79) #(1., 1., 1., 1., 1., 1., 1., 1.)#(1.13, 1.58, 1.33, 1.21, 1.31, 2.25, 1.14, 1.24)
timeSampling = 0.05#5This could be dictated by the DAQ from minimicroscope
...
TimeToReachReward=4#6#220#220 #10 #Time that the animal has to get the reward
...
port = 'COM15'#this must be changed for every board that is utilized
...
mouseTrajectory['BoxDimentions'].append((center,radius,'EnvB'))
...
NumOfRotations = 2
RotationFreq = 30./(NumOfRotations+1)#(in minutes)
NonRewardPeriodStart = 3
...
if FlagRandDeliver==0:#Otherwise maintins the value assigned on TimesRecallVector
...
while (time.time() <= (t_end +0.001)) and (np.mean(AccumPerf[-MinNumTrials:])<MinPerf):    
    ...
    if (np.mean(AccumPerf[-MinNumTrials:])>MinPerf): 
    ...
    p = Process(target=sine_tone_EnvB, args=(toneFreq, toneLength, volume, sample_rate))
    p.start()
    ...
    if (1<len(datatimes['cueTime'])) and (np.mod(len(datatimes['cueTime']),freqPlot)==0):# and (1<len(datatimes['timeErrorPort'])):        
        ...
        if 1<kk:
            ...
            if len (Aux0)>21:
                ax1.text(3.5,0.5,'Perform. last Trials = {:.2f}'.format(np.mean((Aux1[-20:]-Aux1[-21])/(Aux0[-20:]-Aux0[-21]))), fontsize=8)
